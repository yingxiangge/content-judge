...
# ⚠️ **`curiosity` 的判据是「有没有为自己辩护」**（2026-09-10 晚）：
#    原来写「话说尽了、结论闭合 → 低分」，**太抽象，模型执行不了** ——
#    实测 Disney 那条把「规律 + 为什么 + 结论」三段讲全了，仍拿 7 分，
#    比只给一个判断、留了口子的 tibo 那条（6 分、43% 回复率）还高，**排序是反的**。
#    ⇒ 换成可判定的动作：**有没有 argue for it**。模型清楚什么叫论证，
#    而「解释到底」是个程度词，没有边界，只会被忽略。
#
# ⚠️ **`attention` 含「原作者会不会接话」**（2026-09-10 晚老板补）：
#    原作者回复/点赞一条评论，X 会把它推给更多人 —— **曝光的放大器在原作者手里**，
#    而四因子原先只问「陌生人会不会停下」，漏了这一环。
#    实测：`@tibo_maker` 那条有理有据顶了原作者一下，回复/赞比 **43%**；
#    943 赞那条只是漂亮的旁观总结，4.7%；套话那条 0.6%。
#    ⚠️ 这是**相关性**，不是因果 —— 数据库只记回复总数，分不出谁回的，
#    「72 条回复里有没有原作者本人」至今未查证。别把它当定论引用。
FACTORS: dict[str, str] = {
    "relevance":  "是不是在回这条推。跑题、套话、能原样贴在任何帖子下面 → 0",
    "attention":  "在一屏评论区里，会不会有人停下 —— 包括原作者本人会不会想接话。正确但平庸 → 低分",
    "identity":   "读完能否隐约看出这人是干什么的。通用聪明人腔调、无实践视角 → 低分",
    "curiosity":  "会不会让人想「这谁？他还知道什么？」。判据：它有没有为自己的主张辩护 —— 辩护了，读者就没什么可揣摩的 → 低分",
}
